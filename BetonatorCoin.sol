...
// ----------------------------------------------------------------------------
// BetonatorCoin TokenSale. version 1.0
//
...
contract BetonatorCoin is MintableToken{
	string public constant name = "BetonatorCoin";
 ...
 * as they arrive.
 */
contract BetonatorCoinCrowdsale is Ownable {
  using SafeMath for uint256;
  ...
  // how many ETH cost 1000 BETC. rate = 1000 BETC/ETH. It's always an integer!
  //formula for rate: rate = 1000 * (BETC in USD) / (ETH in USD)
  uint256 public rate;
  ...
  function BetonatorCoinCrowdsale(uint256 _rate) public {
    require(_rate > 0);
	require (_rate < 1000);
  ...
  function createTokenContract() internal returns (BetonatorCoin) {
    return new BetonatorCoin();
  }
